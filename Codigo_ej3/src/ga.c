...
// FITNESS secuencial
		...
		double tf = mseconds();
		printf("Fitness secuencial: %lfms\n", tf-ti);

// FITNESS atomic
		 ti = mseconds()
		// recalcula el fitness del individuo
		for(i = 0; i < tam_pob; i++) {
			fitness_atomic(d, poblacion[i], n);
		}
		 tf = mseconds();
		printf("Fitness atomic: %lfms\n", tf-ti);
		
// FITNESS critical
		 ti = mseconds()
		// recalcula el fitness del individuo
		for(i = 0; i < tam_pob; i++) {
			fitness_critical(d, poblacion[i], n);
		}
		 tf = mseconds();
		printf("Fitness critical: %lfms\n", tf-ti);
		
// FITNESS REDUCTION
		 ti = mseconds()
		// recalcula el fitness del individuo
		for(i = 0; i < tam_pob; i++) {
			fitness_reduction(d, poblacion[i], n);
		}
		 tf = mseconds();
		printf("Fitness reduction: %lfms\n", tf-ti);
		// ordena individuos segun la funcion de bondad (menor "fitness" --> mas aptos)
		qsort(poblacion, tam_pob, sizeof(Individuo *), comp_fitness);
...
void fitness_atomic(const double *d, Individuo *individuo, int n)
{
	double fitness = 0.0;
	double valor = 0.0;
	#pragma omp parallel num_threads(2) private(valor)
	{
		#pragma omp for
		for (int i = 0; i<n; i++){
			if (i == (n-1)){
				valor = distancia_ij(d,individuo->array_int[i],individuo->array_int[0], n);
			}
			else{
				valor = distancia_ij(d,individuo->array_int[i],individuo->array_int[i+1], n);
			}
			#pragma omp atomic
			fitness += valor;
		}
